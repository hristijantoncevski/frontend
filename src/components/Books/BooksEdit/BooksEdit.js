                        ...
                        <select name="category" className="form-control" onChange={handleChange}>
                            {props.categories.map((category) => {
                                if(props.book.category !== undefined && props.book.category === category)
                                    return <option selected={props.book.category} key={category} value={category}>{category}</option>
                        ...
                        <select name="author" className="form-control" onChange={handleChange}>
                            {props.authors.map((author) => {
                                if (props.book.author !== undefined && props.book.author.id === author.id)
                                    return <option selected={props.book.author.id} key={author.id} value={author.id}>{author.name + " " + author.surname}</option>
                                else return <option key={author.id} value={author.id}>{author.name + " " + author.surname}</option>
                            })}
                        </select>
                    </div>
                    <div className={"form-group"}>
